...
import type { Document, DatabaseConfig, Query, QueryFunction, Update, UpdateFunction, Acceptable } from './types';
...
export class Database<Schema extends Acceptable<Schema> = Document> {
     ...
     */
    public findOne(query?: Query<Schema> | QueryFunction<Schema>): Schema | null {
     ...
     */
    public findMany(query?: Query<Schema> | QueryFunction<Schema>): Schema[] {
        const { immutable } = this.config;
        if (!isUndefined(query) && !isObject(query) && !isFunction(query)) throw new TypeError('Query must be an object or function');
     ...
     */
    public count(query?: Query<Schema> | QueryFunction<Schema>): number {
        if (!isUndefined(query) && !isObject(query) && !isFunction(query)) throw new TypeError('Query must be an object or function');
